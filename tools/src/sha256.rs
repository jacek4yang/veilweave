// 纯 Rust 实现的 SHA-256（FIPS 180-4）
//
// 安全性基于 SHA-256 的抗碰撞性和单向性。
// 在量子计算机上，Grover 算法将 256 位安全性降低为 128 位，
// 因此本实现提供 **128 位量子安全性**。
...
mod tests {
    ...
    macro_rules! hex_literal {
        ($hex:expr) => {{
            ...
            let hex_str = $hex;
            for i in 0..32 {
                arr[i] = u8::from_str_radix(&hex_str[i * 2..i * 2 + 2], 16).unwrap();
            }
            arr
        }};
    }
